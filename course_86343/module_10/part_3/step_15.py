...
Примечание 2.
Слово — последовательность букв. Кроме слов в тексте могут присутствовать пробелы и знаки препинания .,!?:;-,
...
def main():
    ...
    result = sorted([x for x in result if result[x] == min(result.values())])
    print(result[0])
